...
- **New and improved features:**
  ...
  - **Improved accessibility for links in all components.** After merging [#6441](https://github.com/twitter/bootstrap/pull/6441), link hover states now apply to the `:focus` state as well. This goes for basic `<a***REMOVED***` tags, as well as buttons, navs, dropdowns, and more.
  - Added print utility classes to show and hide content between `screen` and `print` via CSS.
  - Updated input groups to make them behave more like default form controls. Added `display: inline-block;`, increased `margin-bottom`, and added `vertical-align: middle;`  to match `<input***REMOVED***` styles.
...
- **Popovers:**
  ...
  - Plugin no longer inserts popover content into a `<p***REMOVED***`, but rather directly into `.popover-content`.
...
- **Typeahead:**
  ...
  - Hitting escape will place focus back on the `<input***REMOVED***`.
...
- Hero unit now sets basic type styles for the entire component, rather than on `.hero-unit p { ***REMOVED*** ***REMOVED***`.
...
- Removed redundant CSS on `<p***REMOVED***` for `font-family`, `font-size`, and `line-height`.
- Removed redundant `color` declaration from the `<label***REMOVED***` element.
...
- Similarly, scope floated-right dropdowns to immediate children with `.pull-right ***REMOVED*** .dropdown-menu`.
...
- Removed `***REMOVED***` from fluid grid column selectors, meaning every element with a `.span*` class within a `.row-fluid` will use percentage widths instead of fixed-pixels.
...
- Added `***REMOVED***` to most of the button group selectors
...
- All docs pages now have distinct titles, such as <em***REMOVED***Scaffolding &middot; Twitter Bootstrap</em***REMOVED***.
...
- Fixed CSS selector used for `input` and `textarea` grid sizes to properly apply the CSS (was `input ***REMOVED*** .span*` and now is `input.span*`).
