...
function draw(ctx, dimensions, template, ratio) {
	...
	if (ctx.measureText(text).width / width ***REMOVED*** 1) {
...
function render(mode, el, holder, src) {
	...
	var ratio = 1;
	if(window.devicePixelRatio && window.devicePixelRatio ***REMOVED*** 1){
...
function fluid(el, holder, src) {
	...
	***REMOVED***) : theme);
	...
	fluid.style.width = holder.dimensions.width + (holder.dimensions.width.indexOf("%")***REMOVED***0?"":"px");
	fluid.style.height = holder.dimensions.height + (holder.dimensions.height.indexOf("%")***REMOVED***0?"":"px");
	fluid.id = el.id;
...
app.run = function (o) {
	...
	***REMOVED***
	...
	for (var l = images.length, i = 0; i < l; i++) {
		...
		if (src != null && src.indexOf(options.domain) ***REMOVED***= 0) {
